---
title: 'Text mining, sentiment analysis, and visualization on GoT'
date: 'created on 19 May 2025 and updated `r format(Sys.time(), "%d %B, %Y")`'
...
See how that compares to the text in the PDF on Page 7. What has pdftools added and where?
#den har tilføjet \n ved alle linje skift. ved flere linjeskift kommer det gentagende gange
...
Wait, so "lord" is showing up in NRC lexicon as "trust", "disgust", "negative" and "positive"? Let's check:
```{r nrc-confidence}
...
pdf_text <- pdf_text("data/got.pdf")

got_text2 <- tibble(page = 1:length(pdf_text), text = pdf_text) %>%
  unnest_tokens(line, text, token = "lines") %>%
  filter(str_squish(line) != "") %>%
  mutate(linenum = row_number())

got_text2 <- got_text2 %>%
  mutate(chapter = cumsum(str_detect(line, regex("^(PROLOGUE|ARYA|EDDARD|JON|CATELYN|TYRION|DAENERYS|BRAN|SANSA|CHAPTER)", ignore_case = TRUE))))
...
got_sentiment <- got_words %>%
  inner_join(bing, by = "word") %>%
  count(chapter, sentiment) %>%
  pivot_wider(names_from = sentiment, values_from = n, values_fill = 0) %>%
  mutate(net_sentiment = positive - negative)

ggplot(got_sentiment, aes(x = chapter, y = net_sentiment)) +
  geom_col(fill = "darkred") +
  labs(title = "Net Sentiment per Chapter in *A Game of Thrones*",
