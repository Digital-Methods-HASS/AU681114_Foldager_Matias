...
kings1 <- read.csv("data/Danske_kongerække.csv")
glimpse(kings1)

kings2 <- read_csv("data/Danske_kongerække.csv")
glimpse(kings2)

kings3 <- read.csv2("data/Danske_kongerække.csv")
glimpse(kings3)

kings4 <- read_csv2( "data/Danske_kongerække.csv", na="na")
glimpse(kings4)
```
Answer: 
...
kings <- read_csv2("data/Danske_kongerække.csv", na="na")
...
```{r}
kings <- kings %>%
  filter(!is.na(End_of_reign_year)) %>%
  mutate(duration = End_of_reign_year - Start_of_reign_year)
kings %>% 
  select(duration)
```
...
avragekingsduration <- kings%>%
  mutate(mean(duration))
```
...
```{r}
morethanavragekingsduration <- avragekingsduration%>%
  filter(duration>20.5)
morethanavragekingsduration
```

```{r}
count(morethanavragekingsduration)
```
...
```{r}
kingslongestruling <- kings %>%
  filter(!is.na(duration)) %>%
  mutate(days_ruled = duration*365)
kingslongestruling%>%
  arrange(desc(days_ruled))%>%
  slice(1:3)
...
# YOUR CODE
kingsmidyear <- kings%>%
  mutate(midyear = Start_of_reign_year+(End_of_reign_year-Start_of_reign_year)/2)
kingsmidyear
```
